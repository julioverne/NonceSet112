...
uint32_t rk32_via_kmem_read_port(uint64_t kaddr) {
  ...
  if (kmem_read_port == MACH_PORT_NULL) {
    textLog("kmem_read_port not set, have you called prepare_rk?\n");
    sleep(10);
    exit(EXIT_FAILURE);
  }
  ...
  if (err != KERN_SUCCESS) {
    textLog("error setting context off of dangling port: %x %s\n", err, mach_error_string(err));
    sleep(10);
    exit(EXIT_FAILURE);
  }
  ...
  if (err != KERN_SUCCESS) {
    textLog("error calling pid_for_task %x %s", err, mach_error_string(err));
    sleep(10);
    exit(EXIT_FAILURE);
  }
...
uint32_t rk32_via_tfp0(uint64_t kaddr) {
  ...
  err = mach_vm_read_overwrite(tfp0,
                               ...
                               &outsize);
  if (err != KERN_SUCCESS){
    textLog("tfp0 read failed %s addr: 0x%llx err:%x port:%x\n", mach_error_string(err), kaddr, err, tfp0);
    sleep(3);
    return 0;
  }
  
  if (outsize != sizeof(uint32_t)){
    textLog("tfp0 read was short (expected %lx, got %llx\n", sizeof(uint32_t), outsize);
    sleep(3);
    return 0;
  }
  return val;
...
uint32_t rk32(uint64_t kaddr) {
  ...
  textLog("attempt to read kernel memory but no kernel memory read primitives available\n");
  sleep(3);
...
void wkbuffer(uint64_t kaddr, void* buffer, uint32_t length) {
  if (tfp0 == MACH_PORT_NULL) {
    textLog("attempt to write to kernel memory before any kernel memory write primitives available\n");
    sleep(3);
    return;
  }
...
uint64_t kmem_alloc(uint64_t size) {
  if (tfp0 == MACH_PORT_NULL) {
    textLog("attempt to allocate kernel memory before any kernel memory write primitives available\n");
    sleep(3);
    return 0;
  }
  ...
  if (err != KERN_SUCCESS) {
    textLog("unable to allocate kernel memory via tfp0: %s %x\n", mach_error_string(err), err);
    sleep(3);
    return 0;
  }
  return addr;
...
uint64_t kmem_alloc_wired(uint64_t size) {
  ...
  textLog("vm_kernel_page_size: %lx\n", vm_kernel_page_size);
  ...
  textLog("allocated address: %llx\n", addr);
  
  addr += 0x3fff;
  addr &= ~0x3fffull;
  
  textLog("address to wire: %llx\n", addr);
  
  err = mach_vm_wire(fake_host_priv(), tfp0, addr, ksize, VM_PROT_READ|VM_PROT_WRITE);
  if (err != KERN_SUCCESS) {
    textLog("unable to wire kernel memory via tfp0: %s %x\n", mach_error_string(err), err);
    sleep(3);
    return 0;
  }
  return addr;
...
void kmem_free(uint64_t kaddr, uint64_t size) {
  return;
  if (tfp0 == MACH_PORT_NULL) {
    textLog("attempt to deallocate kernel memory before any kernel memory write primitives available\n");
    sleep(3);
    return;
  }
  ...
  if (err != KERN_SUCCESS) {
    textLog("unable to deallocate kernel memory via tfp0: %s %x\n", mach_error_string(err), err);
    sleep(3);
    return;
  }
}
...
void kmem_protect(uint64_t kaddr, uint32_t size, int prot) {
  if (tfp0 == MACH_PORT_NULL) {
    textLog("attempt to change protection of kernel memory before any kernel memory write primitives available\n");
    sleep(3);
    return;
  }
  kern_return_t err;
  ...
  if (err != KERN_SUCCESS) {
    textLog("unable to change protection of kernel memory via tfp0: %s %x\n", mach_error_string(err), err);
    sleep(3);
    return;
  }
}
