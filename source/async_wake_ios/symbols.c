...
int koffset(enum kstruct_offset offset) {
  if (offsets == NULL) {
    textLog("need to call symbols_init() prior to querying offsets\n");
    return 0;
  }
...
uint64_t ksymbols_iphone_7_15B202[] = {
  ...
  0xfffffff00758c618, // KSYMBOL_KtextLog
...
uint64_t ksymbols_ipod_touch_6g_15b202[] = {
  ...
  0xFFFFFFF007559FD0, // KSYMBOL_KtextLog
...
uint64_t ksymbols_iphone_6s_15b202[] = {
  ...
  0xFFFFFFF0075426C4, // KSYMBOL_KtextLog
...
uint64_t ksym(enum ksymbol sym) {
  if (kernel_base == 0) {
    if (!have_kmem_read()) {
      textLog("attempted to use symbols prior to gaining kernel read\n");
      return 0;
    }
...
void offsets_init() {
  ...
  if (err == -1) {
    textLog("failed to detect version (sysctlbyname failed\n");
    return;
  }
  textLog("build_id: %s\n", build_id);
  ...
  if (strcmp(build_id, "15B202") == 0) {
    ...
    offsets = kstruct_offsets_15B202;
    textLog("unknown kernel build. If this is iOS 11 it might still be able to get tfp0, trying anyway\n");
    have_syms = 0;
    return;
  }
  ...
  if (strstr(u.machine, "iPod7,1")) {
    textLog("this is iPod Touch 6G, should work!\n");
    ...
    textLog("this is iPhone 7, should work!\n");
    symbols = ksymbols_iphone_7_15B202;
    have_syms = 1;
  } else if (strstr(u.machine, "iPhone8,1")) {
    textLog("this is iPhone 6s, should work!\n");
    symbols = ksymbols_iphone_6s_15b202;
    have_syms = 1;
  } else {
    textLog("no symbols for this device yet\n");
    textLog("tfp0 should still work, but the kernel debugger PoC won't\n");
    symbols = NULL;
    have_syms = 0;
  }
}
