...
uint64_t find_allproc(void) {
	...
	if (!val) {
		textLog("Failed to calculate x8");
		return 0;
	}
...
addr_t find_amficache(void) {
	...
	if (!bof) {
		textLog("couldn't find the start of the function\n");
		return 0;
	}
	val = calc64(kernel, bof, func, 9);
	if (!val) {
		textLog("couldn't find x9\n");
		return 0;
	}
	return val + kerndumpbase;
}
...
{
	char* foundAt = memmem(kernel, kernel_size, "com.apple.System.boot-nonce", 27);
	while(foundAt!=NULL) {
		
		textLog("Found At: %x", foundAt);
		vm_size_t updateSize = (kernel_size-((foundAt+27)-(const char *)kernel));
		vm_address_t updateAddr = (vm_address_t)foundAt + 27;
		foundAt = memmem((const void *)updateAddr, updateSize, "com.apple.System.boot-nonce", 27);
	}
...
uint64_t find_symbol(const char *symbol)
{
    ...
    for (i = 0; i < hdr->ncmds; i++) {
        ...
        if (cmd->cmd == LC_SYMTAB) {
			textLog("found the symbol table\n");
